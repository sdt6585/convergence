...
export default class Character {
  ...
  /**
   * Calculate skill level based on success checks
   * @param {string} skillName - The name of the skill (will be converted to field name)
   * @returns {number} Skill level from 0-10
   */
  ...
  /**
   * Get skill progression information
   * @param {string} skillName - The name of the skill
   * @returns {object} Object with level progression information
   */
  getSkillProgress(skillName) {
    ...
    // If at max level, return special values
    if (level >= 10) {
      return {
  ...
  /**
   * Get complete skill info including level, progress, and checks
   * @param {string} skillName - The name of the skill
   * @returns {object} Object with level and progression information
   */
  getSkill(skillName) {
    //Skill Name is either the field_name, the name, or the id - try each in that order
  ...
  /**
   * Get an array of all skills with their levels
   * @returns {Array} Array of skill objects with name, level, and progress
   */
  getSkills() {
  ...
  /**
   * Groups skills by category, always separating core skills and always returning an array
   * @returns {Array} Array of category objects: [{ name: 'Core', skills: [...] }, { name: 'Category', skills: [...] }, ...]
   */
  getSkillsByCategory() {
    // Get all skills with metadata and progression
    const allSkills = this.getSkills();

    // Build a map of core skill field_names to their core position
    const coreSkillMap = {};
    ...
    // Separate core skills and categorize the rest
    const coreSkills = [];
    const categories = {};

    for (const skill of allSkills) {
      // Use field_name for matching
      ...
      // Categorize by skill.category
      const category = skill.skill && skill.skill.category ? skill.skill.category : 'Other';
      if (!categories[category]) categories[category] = [];
      categories[category].push(skill);
    }

    // Sort core skills by their core position
    coreSkills.sort((a, b) => a.corePosition - b.corePosition);
    // Sort each category by ordinal_position, then alphabetically by skill name
    ...
    // Build the result array: core first, then sorted categories
    const result = [];
